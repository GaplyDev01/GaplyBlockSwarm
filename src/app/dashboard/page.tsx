            ...
            {selectedToken ? (
              <div>
                <div className="flex items-center mb-4">
                  <div className="h-8 w-8 rounded-full bg-sapphire-800 mr-3 overflow-hidden flex items-center justify-center">
                  ...
                  <div>
                    <h3 className="font-cyber">{selectedToken.name}</h3>
                    <div className="text-xs text-emerald-400/70">{selectedToken.symbol}</div>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-end">
                    ...
                    <div className={`${selectedToken.change24h >= 0 ? 'text-green-400' : 'text-red-400'} text-sm`}>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                  ...
                  <div>
                    ...
                    <div className="font-mono">{selectedToken.decimals}</div>
                  </div>
                </div>
          ...
          <div className="space-y-4 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {tokens.slice(0, 3).map(token => (
                <div key={token.mint} className="glass-card rounded-lg p-4">
                  <div className="text-muted-foreground text-sm mb-1">{token.symbol} Price</div>
                  <div className="text-emerald-400 text-xl font-mono font-bold">
                    {token.price ? `$${token.price.toFixed(2)}` : "Loading..."}
                  </div>
                  {token.change24h !== undefined && (
                    <div className={token.change24h >= 0 ? "text-green-400 text-xs" : "text-red-400 text-xs"}>
              ...
              {tokens.length < 3 && Array.from({length: 3 - tokens.length}).map((_, idx) => (
                <div key={`placeholder-${idx}`} className="glass-card rounded-lg p-4">
                  <div className="text-muted-foreground text-sm mb-1">Loading...</div>
                  <div className="animate-pulse bg-emerald-400/20 h-7 w-24 rounded mt-1"></div>
                  <div className="animate-pulse bg-emerald-400/10 h-4 w-16 rounded mt-2"></div>
                </div>
              ))}
            </div>
          </div>
        ...
        return (
          <div className="space-y-4 p-4">
            ...
            <div className="flex flex-col">
              <div className="glass-card rounded-lg p-4 border border-emerald-500/30 mb-2">
                <p className="font-cyber text-sm text-emerald-400 mb-1">BlockSwarms AI</p>
                <p>Welcome to BlockSwarms. How can I assist you with your Solana wallet today?</p>
              </div>
      ...
      case 'signals':
        return (
          <div className="space-y-4 p-4">
            ...
            {/* Use actual token data to generate signals based on price movement */}
            <div className="flex flex-col space-y-2">
              {tokens.slice(0, 3).map(token => {
                ...
                return (
                  <div key={token.mint} className={`glass-card rounded-lg p-3 border-l-4 ${borderClass}`}>
                    <div className="flex justify-between">
                      <span className="font-semibold">{token.symbol}/USD</span>
                      <span className={signalClass}>{signal}</span>
                    </div>
                    <p className="text-sm text-muted-foreground">{signalText}</p>
                  </div>
                );
              })}
              ...
              {/* If we don't have enough tokens yet, show loading placeholders */}
              {tokens.length < 3 && Array.from({length: 3 - tokens.length}).map((_, idx) => (
                <div key={`signal-placeholder-${idx}`} className="glass-card rounded-lg p-3 border-l-4 border-gray-500 animate-pulse">
                  <div className="flex justify-between">
                    <span className="font-semibold bg-emerald-400/20 h-5 w-20 rounded"></span>
                    <span className="bg-emerald-400/20 h-5 w-12 rounded"></span>
                  </div>
                  <div className="bg-emerald-400/10 h-4 w-48 mt-2 rounded"></div>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <WalletContextProvider>
      <div className="min-h-screen bg-sapphire-900 text-white">
        ...
        {/* Main content */}
        <main className="pt-20 md:pl-16 p-4">
          <div className={`mx-auto ${getMainContentClass()}`}>
            {/* Page header */}
            <div className="mb-8 flex justify-between items-center">
              <h1 className="text-2xl font-cyber text-emerald-400">Dashboard</h1>
            ...
            {!dashboardState.isLoading && dashboardState.error && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-6 text-center">
                <h2 className="text-xl font-cyber text-red-400 mb-2">Error Loading Data</h2>
                <p className="mb-4 text-red-300">{dashboardState.error}</p>
              ...
              <div className="grid grid-cols-12 gap-6">
                {dashboardCards
                  ...
                  .map(card => (
                    ...
                    >
                      <div className="bg-sapphire-800/70 backdrop-blur-sm border border-emerald-400/20 rounded-lg h-full">
                        <div className="p-4 border-b border-emerald-400/20 flex justify-between">
                          <h2 className="font-cyber text-emerald-400">{card.title}</h2>
                          <button 
        ...
        {/* Customize Modal */}
        {showCustomizeModal && (
          <div className="fixed inset-0 bg-sapphire-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-sapphire-800 border border-emerald-400/30 rounded-lg max-w-2xl w-full max-h-[80vh] overflow-auto">
              <div className="p-4 border-b border-emerald-400/20 flex justify-between">
                <h2 className="font-cyber text-emerald-400 flex items-center">
              ...
              <div className="p-6">
                <div className="mb-6">
                  <h3 className="text-lg font-cyber text-emerald-400 mb-2">Dashboard Layout</h3>
                ...
                <h3 className="text-lg font-cyber text-emerald-400 mb-3">Visible Cards</h3>
                  ...
                  {dashboardCards.map(card => (
                    ...
                    >
                      <div className="flex items-center">
                        <div className={`w-8 h-8 rounded-full ${card.visible ? 'bg-emerald-400/20' : 'bg-sapphire-700/50'} flex items-center justify-center mr-3`}>
                          {(() => {
                            switch(card.type) {
                        ...
                        <div>
                          <div className="font-semibold text-emerald-400">{card.title}</div>
                          <div className="text-xs text-emerald-400/50">Size: {card.size}</div>
                        </div>
                      </div>
                ...
                <h3 className="text-lg font-cyber text-emerald-400 mb-3">Add New Card</h3>
                ...
                <div className="mt-8 flex justify-between border-t border-emerald-400/20 pt-6">
                  <Button 
                    variant="outline"
                    onClick={() => {
