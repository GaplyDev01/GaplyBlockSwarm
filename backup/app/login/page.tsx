...
export default function WalletLoginPage() {
  ...
  const handleConnectWallet = async () => {
    try {
      ...
      // Set demo cookie as backup if normal connect fails or is in testing environment
      ...
      if (error instanceof Error) {
        setErrorMessage(`Wallet connection error: ${error.message}`);
      } else {
        setErrorMessage('Failed to connect wallet. Please try again or use Demo Mode.');
      }
      
      // Add a button to continue in demo mode
      document.getElementById('demo-mode-option')?.classList.remove('hidden');
    }
  };
  ...
  return (    <div className="min-h-screen bg-sapphire-900 overflow-hidden bg-tech-pattern">
        ...
        <Wallet className="mr-2 h-5 w-5" />
                  ...
                  >
                    ...
                    id="demo-mode-option"
                    className="w-full mt-2 flex justify-center items-center bg-amber-500/30 hover:bg-amber-500/40 text-amber-100 py-2 rounded-md hidden"
                  >
